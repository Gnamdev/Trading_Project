import api, { API_BASE_URL } from "@/config/api";
import {
  FETCH_COIN_BY_ID_FAILURE,
  FETCH_COIN_BY_ID_REQUEST,
  FETCH_COIN_BY_ID_SUCCESS,
  FETCH_COIN_DETAILS_FAILURE,
  FETCH_COIN_DETAILS_REQUEST,
  FETCH_COIN_DETAILS_SUCCESS,
  FETCH_COIN_LIST_FAILURE,
  FETCH_COIN_LIST_REQUEST,
  FETCH_COIN_LIST_SUCCESS,
  FETCH_COIN_MARKET_CHART_FAILURE,
  FETCH_COIN_MARKET_CHART_REQUEST,
  FETCH_COIN_MARKET_CHART_SUCCESS,
  FETCH_TOP_50_COINS_FAILURE,
  FETCH_TOP_50_COINS_REQUEST,
  FETCH_TOP_50_COINS_SUCCESS,
  SEARCH_COIN_FAILURE,
  SEARCH_COIN_REQUEST,
  SEARCH_COIN_SUCCESS,
} from "./ActionType";
import axios from "axios";

export const getCoinList = (page) => async (dispatchEvent) => {
  dispatchEvent({ type: FETCH_COIN_LIST_REQUEST });

  // const BASE_URL = "http://localhost:8080"

  try {
    const { data } = await axios.get(`${API_BASE_URL}/coins?page=${page}`);

    console.log("coin list ->", data);

    dispatchEvent({ type: FETCH_COIN_LIST_SUCCESS, payload: data });
  } catch (error) {
    dispatchEvent({ type: FETCH_COIN_LIST_FAILURE, payload: error.message });
    console.log(error);
  }
};

export const getTop50CoinList = () => async (dispatchEvent) => {
  dispatchEvent({ type: FETCH_TOP_50_COINS_REQUEST });

  // const BASE_URL = "http://localhost:8080"

  try {
    const { data } = await axios.get(`${API_BASE_URL}/coins/top50`);

    console.log("top 50 coin list ->", data);

    dispatchEvent({ type: FETCH_TOP_50_COINS_SUCCESS, payload: data });
  } catch (error) {
    dispatchEvent({ type: FETCH_TOP_50_COINS_FAILURE, payload: error.message });
    console.log(error);
  }
};

export const fetchMarketChart =
  ({ coinId, days, jwt }) =>
  async (dispatchEvent) => {
    dispatchEvent({ type: FETCH_COIN_MARKET_CHART_REQUEST });

    try {
      const response = await api.get(`/coins/${coinId}/chart?days=${days}`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });

      dispatchEvent({
        type: FETCH_COIN_MARKET_CHART_SUCCESS,
        payload: response.data,
      });
    } catch (error) {
      dispatchEvent({
        type: FETCH_COIN_MARKET_CHART_FAILURE,
        payload: error.message,
      });
      console.log(error);
    }
  };

export const fetchCoinById = (coinId) => async (dispatchEvent) => {
  dispatchEvent({ type: FETCH_COIN_BY_ID_REQUEST });

  try {
    const response = await axios.get(`${API_BASE_URL}/coins/${coinId}`);

    dispatchEvent({
      type: FETCH_COIN_BY_ID_SUCCESS,
      payload: response.data,
    });
  } catch (error) {
    dispatchEvent({
      type: FETCH_COIN_BY_ID_FAILURE,
      payload: error.message,
    });
    console.log(error);
  }
};

export const fetchCoinDetails =
  ({ coinId, jwt }) =>
  async (dispatchEvent) => {
    dispatchEvent({ type: FETCH_COIN_DETAILS_REQUEST });

    try {
      const response = await api.get(`/coins/details/${coinId}`, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });

      dispatchEvent({
        type: FETCH_COIN_DETAILS_SUCCESS,
        payload: response.data,
      });
    } catch (error) {
      dispatchEvent({
        type: FETCH_COIN_DETAILS_FAILURE,
        payload: error.message,
      });
      console.log(error);
    }
  };

export const searchCoin = (keyword) => async (dispatchEvent) => {
  dispatchEvent({ type: SEARCH_COIN_REQUEST });

  try {
    const response = await api.get(`/coins/search?q=${keyword}`);

    dispatchEvent({
      type: SEARCH_COIN_SUCCESS,
      payload: response.data,
    });
  } catch (error) {
    dispatchEvent({
      type: SEARCH_COIN_FAILURE,
      payload: error.message,
    });
    console.log(error);
  }
};
